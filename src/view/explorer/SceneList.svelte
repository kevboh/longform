  ...
  */
    ...
    function itemsFromScenes(
        ...
        scenes.forEach(({ title, indent, numbering }, index) => {
            ...
            const nextScene =
                index < scenes.length - 1 ? scenes[index + 1] : false;
            const item = {
                id: title,
                name: title,
                indent: indent,
    ...
    undoManager.on((type, _evt, _ctx) => {
        ...
        if (type === "undo") {
            ...
            undoIndex = Math.max(
                Math.min(undoIndex + 1, sceneHistory.length - 1),
                0
            );
        } else {
        ...
        if (
            ...
            ($drafts[currentDraftIndex] as MultipleSceneDraft).scenes =
                newScenes;

            new Notice(
                `${type === "undo" ? "Undid" : "Redid"} scene reordering`
            );
        }
        return false;
    });
            ...
            >
                ...
                >
                    {#if $pluginSettings.numberScenes}
                        <span class="longform-scene-number"
                            >{numberLabel(item)}</span
                        >
                    {/if}
                    <div
                        id={`longform-scene-${item.name}`}
                        style="display: inline;"
                        on:keydown={item.name === editingName
                            ? onKeydown
                            : null}
            ...
            <div class="longform-unknown-inner">
                ...
                <div>
                    <button
                        ...
                        on:click={() => doWithAll("add")}>Add all</button
                    >
                    <button
                            ...
                            <div class="longform-unknown-file">
                                ...
                                <div>
                                    <button
                                        class="longform-unknown-add"
                                        on:click={() =>
                                            doWithUnknown(fileName, "add")}
                                        >Add</button
                                    >
                                    <button
                                        class="longform-unknown-ignore"
                                        on:click={() =>
                                            doWithUnknown(fileName, "ignore")}
                                        >Ignore</button
                                    >
                                </div>
                            </div>
